...
if __name__ == "__main__":
	...
	#ccc=[argv[1]] 3 6 7 11 13 16 17;
	for chrom in [22,21,20,19,18,4,5,8,9,10,14,15]:  # range(2,20):
		print("Working on chr ",chrom)
	
		
		input_vcf = "10x/10x.vcf" 
		chr_vcf = "10x/"+str(chrom)+"/"+str(chrom)+".vcf"		
...
#		subprocess.call("mkdir 10x/"+str(chrom), shell=True)
		## subprocess.call("grep \"#\" "+input_vcf+" | sed \'s\/Type=Integer,Description=\"ID of Phase Set for Variant\"/Type=String,Description=\"Phase set in which this variant falls\"/\' >"+chr_vcf, shell=True)	

#		subprocess.call("grep \"#\" "+input_vcf+">"+chr_vcf, shell=True)   

#		subprocess.call("grep \"^chr"+str(chrom)+"\\b\" "+input_vcf+" |sed s/chr"+str(chrom)+"/"+str(chrom)+"/g |grep -v \"TYPE=ins\" | grep -v  \"TYPE=snp,\" | grep -v \"TYPE=del\" | grep -v \"9552805\" >>  "+chr_vcf, shell=True)	
