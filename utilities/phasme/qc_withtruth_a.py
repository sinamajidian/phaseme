...
def report_qc(report_qc_address, id_blocks, qual_blocks, allele_blocks, stats_vcf):
    ...
    #file_report_qc.write("##Chromosome\tN50(Kb),\t\'Avg. phase block quality\',\t'Number of heterozygous variants ',\t'Number of non-phased heterozygous variants',\t'Number of homozygous variants',\t'Phase rate' \n")    
    file_report_qc.write('chr'+str(chrom)+",\t"+str(np.round(n50/1000,1))+",\t"+str(np.round(np.mean(q_blocks),3))+",\t"+str(hetrozygous_phased)+",\t"+str(hetrozygous_nonphased)+",\t"+str(homozygous0_num+homozygous1_num)+',\t'+str(np.round(phase_rate,3))+"\n")
...
if __name__ == "__main__":
    ...
    for Forward_or_both in  ['forward', 'both']:
...
#type_data+'_son/'+str(chrom)+'/'+str(chrom)+'_report_mismatches_'+Forward_or_both+'.txt'
    ...
    report_qc_address=argv[1][:-4]+'_report_qc.txt' # type_data+'_son/'+str(chrom)+'/'+str(chrom)+'_report_qc_'+Forward_or_both+'.txt'
