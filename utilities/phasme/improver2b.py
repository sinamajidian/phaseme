    ...
    for var_i in range(len(allele_block)):
        ...
        if var_pos >= lower_bound and  var_pos <= upper_bound:
            ...
            if var_pos in pop_inf_dic.keys():
                ...
                if var_pos==16962868:
                    print("here2")
                    print(vars_identical_phase,vars_opposite_phase)
...
if __name__ == "__main__":
    ...
    file_mismatches_address = vcf_file_address[:-4]+'_report_mismatches_'+Forward_or_both+'.txt'
...
#type_data+'_son/'+str(chrom)+'_report_mismatches_'+Forward_or_both+'.txt'
    ...
    vcf_file_improved_address = vcf_file_address[:-4]+'_improved_.90_cut.vcf'
