...
def run_shapeit_graph(chr_vcf, data_1000G_address, chrom): # run per chromosom
	...
	shapeit_check= SHAPEIT+" -check --exclude-ind gwas.subset.inds  --input-vcf "+chr_vcf+" -R "+data_1000G_address+"1000GP_Phase3_chr"+str(chrom)+".hap.gz "+data_1000G_address+"1000GP_Phase3_chr"+str(chrom)+".legend.gz  "+data_1000G_address+"1000GP_Phase3.sample --output-log files_"+str(chrom)+"/shapeit_check >> files_"+str(chrom)+"/samples.log"

	subprocess.call(shapeit_check, shell=True)

	shapeit_generate_graph= SHAPEIT+" --exclude-ind gwas.subset.inds --input-vcf  "+chr_vcf+" -R "+data_1000G_address+"1000GP_Phase3_chr"+str(chrom)+".hap.gz "+data_1000G_address+"1000GP_Phase3_chr"+str(chrom)+".legend.gz "+data_1000G_address+"1000GP_Phase3.sample  -M "+data_1000G_address+"genetic_map_chr"+str(chrom)+"_combined_b37.txt  --output-log files_"+str(chrom)+"/shapeit_graph --output-graph "+chr_vcf[:-4]+".graph  >> files_"+str(chrom)+"/samples.log" # --exclude-snp  files_"+str(chrom)+"/shapeit_check.snp.strand.exclude
...
if __name__ == "__main__":
	...
	NEIGHBOURS =  20  #10			   # number of neighbour variants to be checked 
	THRESH=0.90        		  # the extent of  between samples 
	num_samples =  500	#500 # number that we sample the haplotype graph (output of shapeit)
	...
	chrr=int(argv[1])
	for chrom in [chrr] : #reversed(range(1,23)):
		print("Working on chr ",chrom)
		
		#input_vcf = "ont/ont.vcf" 
#		chr_vcf = "ont/"+str(chrom)+"/"+str(chrom)+".vcf"	
		chr_vcf=argv[2] #"/home/ssm/Documents/phaseme/data1/1000g_vcf/"+str(chrom)+"_fake.vcf" #"22.vcf"
